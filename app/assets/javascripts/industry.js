...
// 子カテゴリーの表示作成
  function appendChildrenBox(insertHTML) {
    ...
    childrenSelectHtml =`<select class="Input-js__text" id="child-wrapper" name="experience[industry_id]"><option value="">選択してください</option>
                          ${insertHTML}
                        </select>`;
    $('.Input-js').append(childrenSelectHtml);
  }
...
// 親カテゴリー選択後のイベント
  $("#parent-category").on('change', function () {
    ...
    if (parentID != "") { //親カテゴリーが初期値でないことを確認してAjax通信を開始
      $.ajax( {
        type: 'GET',
        url: '/tops/get_category_children',
